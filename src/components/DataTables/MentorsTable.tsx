...
const MentorsTable: FC = () => {
  ...
  const data: Employee[] = useMemo(() => {
    return (
      usersByRole?.mentors?.map((mentors) => ({
        id: mentors.id,
        name: mentors.fullName || mentors.name || "N/A",
        role: mentors.role || "N/A",
        email: mentors.email || "N/A",
        phone: mentors.phoneNumber || "N/A",
      })) || []
    );
  }, [usersByRole]);
        ...
        <tbody {...getTableBodyProps()}>
          {page.map((row) => {
            ...
            return (
              <tr
                onClick={() => {
                  router.push(`/deck/mentor/${row.original.id}`);
                }}
