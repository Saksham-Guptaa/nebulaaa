...
const InfluencersTable: FC = () => {
  ...
  const data: Employee[] = useMemo(() => {
    return (
      usersByRole?.influencers?.map((influencers) => ({
        id: influencers.id,
        name: influencers.fullName || influencers.name || "N/A",
        role: influencers.role || "N/A",
        email: influencers.email || "N/A",
        phone: influencers.phoneNumber || "N/A",
      })) || []
    );
  }, [usersByRole]);
        ...
        <tbody {...getTableBodyProps()}>
          {page.map((row) => {
            ...
            return (
              <tr
                onClick={() => {
                  router.push(`/deck/influencer/${row.original.id}`);
                }}
